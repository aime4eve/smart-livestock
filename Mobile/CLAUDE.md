...
│   ├── routes/              # 路由：auth, me, dashboard, map, alerts, fences, tenants, profile
...
```
main.dart (async)
  → ApiCache.instance.init('owner')   // 预加载：并发 GET 6 个 API 端点
  → runApp(DemoApp)
...
基础端点：auth(1) + me(1) + dashboard(1) + map(1) + alerts(5) + fences(4) + tenants(4) + profile(1)；数智孪生扩展：`/api/twin/*`（overview、fever/digestive/estrus 列表与详情、epidemic summary/contacts）
...
MVP 阶段将用 FastAPI 替换 Mock Server，遵循以下分层：

- `models/*` - 只负责持久化结构，不写业务规则
- `schemas/*` - 只负责接口契约（DTO）
