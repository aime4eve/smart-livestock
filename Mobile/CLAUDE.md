...
**当前阶段**: 统一商业模型 Phase 2a 已完成（B端管理后台 + 多牧场 + 订阅基础设施），Phase 2b 设计中。前端使用 Flutter + Riverpod，后端 Mock API 使用 Node.js + Express（端口 3001）。APP_MODE=live 时前端通过 HTTP 调用 Mock Server，APP_MODE=mock 时使用本地假数据。
...
- **MVP 后端** (待实现): Spring Boot 3.x + Java 17 + PostgreSQL + Redis + RocketMQ（详见 [MVP 后端设计规格](../docs/superpowers/specs/2026-05-06-mvp-backend-design.md)）
...
当前功能模块：`admin`、`alerts`、`b2b_admin`、`dashboard`、`devices`、`digestive`、`epidemic`、`estrus`、`farm_switcher`、`fence`、`fever_warning`、`highfi`、`livestock`、`mine`、`stats`、`subscription`、`tenant`、`twin_overview`、`worker_management`
...
MVP 阶段将用 **Spring Boot 3.x** 替换 Mock Server，采用 DDD 洋葱架构 + 充血模型。完整设计详见 [MVP 后端设计规格](../docs/superpowers/specs/2026-05-06-mvp-backend-design.md)。
