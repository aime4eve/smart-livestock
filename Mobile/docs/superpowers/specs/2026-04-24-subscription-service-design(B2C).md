...
> **勘误（2026-04-28）**：以下条目已被统一商业模型设计规格（`SL-BIZ-2026-001` v1.3）覆盖，实施时以统一规格为准：
>
> | # | 本规格原文 | 统一规格覆盖内容 | 涉及位置 |
> |---|-----------|----------------|---------|
> | S1 | `getSubscriptionTier(tenantId)` 查 subscriptionStore | `getEffectiveTier(farmTenantId)` 查 `tenant.entitlementTier`（经 subscription 状态校验） | Section "后端架构" shapingMiddleware 代码块 |
> | S2 | per-route 注册中间件 | 全局注册（auth → farmContext → shaping） | Section "中间件注册顺序" |
> | S3 | 超额公式：批次定价（每 50 头 +¥50） | 每头定价（¥2/头） | Section "订阅层级" 计费示例 |
> | S4 | `calculatedPrice: int // 单位：分` | 3 子字段（`calculatedDeviceFee` + `calculatedTierFee` + `calculatedTotal`），单位：元 | Section "数据模型" SubscriptionStatus |
> | S5 | `monthlyPrice`/`perUnitPrice` 单位：分 | **所有价格字段统一使用元**（与 `calculatedTotal` 一致），实施时需同步修改 `SubscriptionTier.monthlyPrice` 和 `perUnitPrice` 的值 | Section "数据模型" SubscriptionTier |
> | S6 | `feature-flags.js` schema 无 `requiredDevices` | 每个 feature key 新增 `requiredDevices` 字段 | Section "`data/feature-flags.js` schema" |
> | S7 | Shaping tier 来源 `req.user?.tenantId` | `req.activeFarmTenantId` + `getEffectiveTier()` | Section "后端架构" shapingMiddleware |
> | S8 | checkout/renew 响应仅含 tier 费用 | 新增 `calculatedDeviceFee` + `calculatedTotal`，renew 时重算设备月费 | Section "订阅管理 API" |
> | S9 | `SubscriptionTier` 模型 `perUnitPrice`/`perUnitSize`（批次定价字段） | 每头定价，字段语义需更新（`perUnitPrice` 含义从"每批次价格"改为"每头价格"） | Section "数据模型" SubscriptionTier |
