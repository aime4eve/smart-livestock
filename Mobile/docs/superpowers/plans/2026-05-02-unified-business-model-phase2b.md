...
- [x] **Step 2: 注册到 registerApiRoutes.js**
...
- [x] **Step 3: 在 server.js ROUTE_DEFINITIONS 区段追加 E6 端点行**
...
- [x] **Step 6: RolePermission 新增 canManageContracts()**
...
- [x] **Step 1: 修改 canManageFarm() 新增 b2b_admin 分支**
...
- [x] **Step 2: 运行 worker-routes 测试验证**
...
- [x] **Step 3: RolePermission 新增 canManageSubfarmWorkers()**
...
- [x] **Step 1: 实现 subscriptionServiceRoutes.js**
...
- [x] **Step 2: auth.js PUBLIC_PATHS 扩展**
...
- [x] **Step 2: 在 registerApiRoutes.js 注册 revenueRoutes + server.js ROUTE_DEFINITIONS 追加**
...
- [x] **Step 2: 创建 platform_admin 对账看板页面**

全局结算周期列表（状态标签）+ 详情（分润汇总表 + 手动触发结算按钮）+ 确认按钮。

- [x] **Step 3: 创建 b2b_admin 对账页面**
...
- [x] **Step 5: RolePermission 新增 canViewRevenue() 和 canCalculateRevenue()**
...
- [x] **Step 3: 编写 rateLimit 中间件测试**

- [x] **Step 4: 修改 farmContextMiddleware — api_consumer 显式分支**
...
- [x] **Step 5: 修改 shapingMiddleware — apiTier 分叉**
...
- [x] **Step 2: 取消 server.js 中 G1.2-TODO 注释 + ROUTE_DEFINITIONS 追加**
...
- [x] **Step 1: 实现 apiAuthorizationRoutes.js**
...
- [x] **Step 2: platform_admin 后台 API 授权管理页面**

授权列表（状态筛选）+ 审批操作（通过/拒绝/撤销）。

- [x] **Step 3: owner App MinePage 新增入口**

"API 授权管理"入口，按 farm 维度展示授权请求 + 审批操作。

- [x] **Step 4: 注册路由 + 扩展导航**

- [x] **Step 5: RolePermission 新增 canReviewApiAuthorizations() / canRequestApiAuthorization()**
...
- [x] **Step 2: 实现 AppLayout 组件（顶部导航 + 侧边栏）**

侧边栏：仪表盘 / API Keys / 端点文档 / 授权管理 / 设置

- [x] **Step 3: 实现 API 客户端 (Fetch 封装)**
...
- [x] **Step 1: LoginView — 输入凭据登录**

Mock 环境直接使用 `mock-token-api-consumer`，调用 `/api/v1/me` 确认身份。

- [x] **Step 2: RegisterView — 占位页面**

显示"请联系平台管理员申请 API 访问权限"。

- [x] **Step 3: DashboardView — 用量仪表盘**

显示调用量、配额剩余、使用趋势（通过 `/api/v1/subscription/usage` 类端点）。

- [x] **Step 4: ApiKeysView — Key 查看/轮换**

显示 Key 前缀 + 后 4 位，支持轮换操作。组件：ApiKeyDisplay。

- [x] **Step 5: EndpointsView — API 文档浏览**

按 tier 分组展示可用端点。

- [x] **Step 6: AuthorizationsView — 授权申请/状态**

显示自己的授权申请列表 + 状态 + 提交新申请表单。

- [x] **Step 7: SettingsView — 账户设置**
...
- [x] **Step 1: 编写 LoginView 测试**

```javascript
// 测试登录表单渲染、输入绑定、提交逻辑
```

- [x] **Step 2: 编写 DashboardView 测试**

```javascript
// 测试仪表盘数据显示（mock API 响应）
```

- [x] **Step 3: 编写 ApiKeysView 测试**
...
- [x] **Step 5: 配置 vite.config.js 支持测试**
...
- [x] **Step 3: 编写新页面 widget smoke 测试（可选）**
...
- [x] **Step 1: platform_admin 登录**

验证后台新增页面：对账看板、合同管理、订阅服务管理、API 授权管理。

- [x] **Step 2: b2b_admin 登录**
...
- [x] **Step 4: api_consumer 登录开发者门户**

验证登录 → 仪表盘 → API Keys → 授权申请。

- [x] **Step 5: curl 验证 Open API 端点**
