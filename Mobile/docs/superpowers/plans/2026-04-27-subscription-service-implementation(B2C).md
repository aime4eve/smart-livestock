...
- [x] **Step 1: 写测试 `backend/test/feature-flags.test.js`**
...
- [x] **Step 3: 实现 `backend/data/feature-flags.js`**
...
- [x] **Step 1: 实现 `backend/data/subscriptions.js`**
...
- [x] **Step 2: 运行 quick verification**
...
- [x] **Step 1: 写测试 `backend/test/response-shaping.test.js`**
...
- [x] **Step 3: 实现 `backend/middleware/feature-flag.js`**
...
- [x] **Step 1: 写测试 `backend/test/subscription-api.test.js`**
...
- [x] **Step 3: 实现 `backend/routes/subscription.js`**
...
- [x] **Step 1: 写集成测试 `backend/test/tier-access-integration.test.js`**
...
- [x] **Step 2: 运行测试确认失败（当前路由未注册 shaping）**
...
- [x] **Step 3: 修改 `backend/routes/map.js`**
...
- [x] **Step 4: 修改 `backend/routes/fences.js`**
...
- [x] **Step 5: 修改 `backend/routes/alerts.js`**
...
- [x] **Step 6: 修改 `backend/routes/dashboard.js`**
...
- [x] **Step 7: 修改 `backend/routes/twin.js`**
...
- [x] **Step 1: 修改 `backend/routes/tenants.js` — POST handler 集成**
...
- [x] **Step 2: 修改 `backend/routes/registerApiRoutes.js`**
...
- [x] **Step 1: 写测试 `test/core/subscription_tier_test.dart`**
...
- [x] **Step 3: 实现 `lib/core/models/subscription_tier.dart`**
...
- [x] **Step 1: 写测试 `test/widgets/locked_overlay_test.dart`**
...
- [x] **Step 3: 实现 `lib/widgets/locked_overlay.dart`**
...
- [x] **Step 1: 写 Controller 测试 `test/features/subscription/subscription_controller_test.dart`**
...
- [x] **Step 6: 实现 `subscription_controller.dart`**
...
- [x] **Step 0b: 写 `feature_comparison_table` 测试**
...
- [x] **Step 0c: 写 `usage_progress_bar` 测试**
...
- [x] **Step 2: 实现 `feature_comparison_table.dart`**

功能对比表：行=feature category，列=tier，支持滚动。

- [x] **Step 3: 实现 `usage_progress_bar.dart`**

用量进度条：当前使用量 / 套餐限制，超量红色警告。

- [x] **Step 4: 运行测试确认通过**

- [x] **Step 5: 实现 `subscription_plan_page.dart`**
...
- [x] **Step 6: 实现 `subscription_checkout_page.dart`**
...
- [x] **Step 7: 运行 flutter analyze + 测试**
...
- [x] **Step 1: 修改 `lib/app/app_route.dart`**
...
- [x] **Step 2: 修改 `lib/app/app_router.dart`**
...
- [x] **Step 3: 新增 `refreshSubscription()` 方法**
...
- [x] **Step 4: checkout/cancel/renew 远程方法并触发重载**
...
- [x] **Step 1: 实现 `subscription_status_card.dart`**
...
- [x] **Step 2: 运行测试确认失败** → 实现 `subscription_renewal_banner.dart`
...
- [x] **Step 3: 实现 `subscription_expiry_dialog.dart`**
...
- [x] **Step 4: 修改 `twin_overview_page.dart`**

在页面顶部 Column children 最前面添加 `const SubscriptionRenewalBanner()`。

- [x] **Step 5: 修改 `app_router.dart` 或 `demo_shell.dart` — 登录后弹窗检查**
...
- [x] **Step 1: 在 `mock_subscription_repository.dart` 中实现 `applyMockShaping()`**
...
- [x] **Step 2: 在各 Mock Repository 返回数据前调用**
...
- [x] **Step 3: `demo_seed.dart` 新增订阅假数据**
...
- [x] **Step 1: 实现 `tier_visibility_test.dart`**

验证 basic 用户孪生页面：发热/消化正常、发情/疫病有锁。

- [x] **Step 2: 实现 `subscription_checkout_flow_test.dart`**

验证支付流程：选择套餐 → 确认 → loading → 成功 → 状态更新。

- [x] **Step 3: 实现 `subscription_cancel_flow_test.dart`**

验证取消流程：确认弹窗 → 取消 → status='cancelled'。

- [x] **Step 4: 实现 `subscription_renewal_reminder_test.dart`**

验证到期提醒：7 天 warning、3 天 critical、0 天 expired。

- [x] **Step 5: 运行 flutter analyze**
...
- [x] **Step 2: 运行前端全部测试 + analyze**

```bash
cd Mobile/mobile_app && flutter analyze && flutter test
```

- [x] **Step 3: 启动 Mock Server 验证端点**
