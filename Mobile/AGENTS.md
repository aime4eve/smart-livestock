...
via IoT devices (GPS trackers, rumen capsules, accelerometers). Current stage: **high-fidelity
Demo → MVP transition**. Uses local mock data (`APP_MODE=mock`) or a Node.js mock server
(`APP_MODE=live`). No real backend yet.
...
Mock server lives in `backend/` (Node.js + Express 5, port 3001). `infra/` is not yet implemented.

## Architecture Overview

```
mobile_app/lib/
├── app/           # App shell, router, session, mode switching
├── core/
│   ├── models/    # Domain models (demo_role, demo_models, view_state)
...
├── features/
│   ├── pages/     # Page widgets (twin_overview_page, map_page, alerts_page, …)；legacy `dashboard_page` 仍保留于 `/dashboard`
│   ├── auth/      # Login page
...
└── widgets/       # Shared widgets (metric_card, …)
```
...
| File | Purpose |
|------|---------|
| `app/app_route.dart` | `AppRoute` enum — single source of truth for paths, names, labels |
...
| `core/models/view_state.dart` | `ViewState` enum for demo state switching |
...
3. **Live implementation** in `data/live_*.dart` — placeholder for real API calls.
...
|------|-----------------|--------|
| 牧场主 (Owner) | `owner` | All pages + admin |
| 牧工 (Worker) | `worker` | Dashboard, Map, Alerts, Mine, Fence |
| 运维 (Ops) | `ops` | `/ops/admin` only, no bottom nav |

Permission checks go through `RolePermission` static methods.

## Key Constraints

- **No real backend calls** — all data is local mock from `DemoSeed` / `mock_*_repository.dart`.
