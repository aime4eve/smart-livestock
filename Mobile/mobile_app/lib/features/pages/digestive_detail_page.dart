...
import 'package:hkt_livestock_agentic/features/ai_anomaly/presentation/widgets/anomaly_history_chart.dart';
...
class DigestiveDetailPage extends ConsumerWidget {
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(
                height: 120,
                child: Center(
                  child: Icon(Icons.grid_on, size: 48, color: AppColors.border),
                ),
              ),
            ],
          ),
                    ...
                    child: GridView.builder(
                      ...
                      itemBuilder: (ctx, i) {
                        ...
                        return Tooltip(
                          ...
                          message:
                              '${cell.hour}:00 · ${intensity.toStringAsFixed(1)}',
                          child: Container(
                            decoration: BoxDecoration(
                              color: isAbnormal
                                  ? AppColors.danger.withValues(
                                      alpha:
                                          0.3 + (intensity / 100).clamp(0, 0.7),
                                    )
  ...
  Widget _buildStatusCards(DigestiveDetailData detail, AppLocalizations l10n) {
    ...
    return Row(
      children: [
        _statCard(
          l10n.digestiveCurrentFreq,
          '${validatedReadings.isEmpty ? detail.motilityBaseline.toStringAsFixed(1) : validatedReadings.last.frequency!.toStringAsFixed(1)}$unit',
          AppColors.danger,
        ),
        const SizedBox(width: 8),
        _statCard(
          l10n.digestiveBaselineFreq,
          '${detail.motilityBaseline.toStringAsFixed(1)}$unit',
          AppColors.textSecondary,
        ),
        const SizedBox(width: 8),
        _statCard(
          l10n.digestiveStatus,
          motilityStatusLabel(l10n, detail.status),
          detail.status == 'ABNORMAL' ? AppColors.danger : AppColors.warning,
        ),
      ],
    );
  }
          ...
          children: [
            Row(
              children: [
                ...
                const Spacer(),
                Text(
                  l10n.latestDataAt(formatMdhm(latestWithFrequency.timestamp)),
                  style: const TextStyle(
                    fontSize: 11,
                    color: AppColors.textSecondary,
                  ),
