...
- [ ] flutter_map_mbtiles 包与 flutter_map 8.x 的兼容性验证（需 tech spike）
- [ ] tileserver-gl 部署方案：独立容器还是嵌入 smart-livestock-server
- [ ] MBTiles 增量更新策略：全量替换 vs 差异补丁（影响后端 API 设计）
...
│                                                         │
│  ┌──────────────┐    ┌────────────────────────────┐    │
│  │ tileserver-gl │───▶│ MBTilesGenerator           │    │
│  │ (Docker sidecar)│   │  按牧场 bbox 生成 MBTiles   │    │
│  └──────────────┘    │  zoom 11-15, PNG 256x256   │    │
│                      └────────────────────────────┘    │
│                                                         │
│  ┌──────────────────────────────────────────────────┐   │
│  │ GET /api/v1/farms/{farmId}/offline-map           │   │
│  │   → 下载 MBTiles 文件（支持 Range 断点续传）        │   │
│  │ HEAD /api/v1/farms/{farmId}/offline-map          │   │
│  │   → 获取文件大小、版本号、MD5                       │   │
│  └──────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────┘
```
...
- **客户端存储**：`getApplicationSupportDirectory()/mbtiles/{farmId}.mbtiles`，避开 Android Scoped Storage 限制
- **TileProvider 链**：Network → MBTiles 回退，网络请求失败时自动读取本地瓦片，无闪烁
...
|------|------------|------------|
| flutter_map_mbtiles 包与 flutter_map 8.x 不兼容 | M | 自写 MBTilesTileProvider（~80 行：SQLite 读取 + TMS Y 轴翻转 + MemoryImage） |
...
| 首次下载 200MB+ 体验差 | L | WiFi 引导 + HTTP Range 断点续传 |
...
| 2 | 后端 MBTiles 生成 | tileserver-gl 部署 + MBTiles 生成 API + 下载端点 | pending | with 3 | 1 | - |
| 3 | Flutter 离线瓦片渲染 | OfflineTileProvider 优先级链 + 在线/离线自动切换 | pending | with 2 | 1 | - |
| 4 | 离线数据持久化 | 围栏 + 牲畜最后位置本地缓存 + 离线渲染 | pending | - | 2, 3 | - |
| 5 | MBTiles 管理界面 | 下载/进度/断点续传/过期检查/删除 UI | pending | - | 4 | - |
