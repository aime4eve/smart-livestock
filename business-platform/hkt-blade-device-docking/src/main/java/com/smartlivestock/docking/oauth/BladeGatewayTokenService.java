...
/**
 * Exchanges an OAuth2 access_token from blade auth using grant_type=openapi.
 * Endpoint: POST /oauth2/token (verified against real blade at 172.22.4.17:8108).
 *
 * Key differences from open-platform-dev OpenApiGatewayTokenService:
 * - token-uri points to blade auth /oauth2/token (not gateway /api/auth/oauth2/token)
 * - adds Tenant-Id header (blade requires it on all requests)
 */
@Slf4j
@Service
public class BladeGatewayTokenService {

    private final BladeOAuth2Properties props;
    ...
    public BladeGatewayTokenService(BladeOAuth2Properties props,
                                    @Qualifier("bladeOAuth2RestTemplate") RestTemplate restTemplate,
    ...
    void logOauth2Binding() {
        if (isReady()) {
            log.info("blade.oauth2 ready, token-uri={}", props.getTokenUri());
        } else {
            log.warn("blade.oauth2 not ready: {}", describeWhyNotReady());
        }
    }
    ...
    public String describeWhyNotReady() {
        if (!props.isEnabled()) {
            return "blade.oauth2.enabled is false";
        }
        if (!StringUtils.hasText(props.getTokenUri())) {
            return "blade.oauth2.token-uri not configured";
        }
        if (!StringUtils.hasText(props.getClientId())) {
            return "blade.oauth2.client-id not configured";
        }
        if (!StringUtils.hasText(props.getClientSecret())) {
            return "blade.oauth2.client-secret not configured or empty";
        }
        return "unknown";
    }

    /**
     * Exchange token by user id; cached and auto-refreshed near expiry.
     */
    ...
    private CachedToken fetchToken(String userId) {
        ...
        // blade requires Tenant-Id header on all requests including OAuth2
