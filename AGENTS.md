...
**blade 平台环境对应**：dev stack ↔ blade dev（172.21.2.41），test stack ↔ blade test（172.22.4.17）。地址默认值写在各自 compose 文件的 `AGENTIC_PLATFORM_*` 变量中，密钥在远程 `.env` / `.env.dev` 手动维护。本项目作为 blade 外部调用方走 URL 直连，不注册 Nacos。注意 blade dev/test 是两套独立平台：OAuth2 client（`hkt_openapi`）相同，但服务账号 `service-user-id` 各自独立（dev=`2079382969422938112`，test=`2074385063398711296`），新建环境需按 `business-platform/hkt-blade-device-docking/README.md` 的自助流程在该平台上创建服务账号。设计文档：`docs/superpowers/specs/2026-07-21-blade-env-mapping-design.md`
...
| **blade 环境映射设计** | `docs/superpowers/specs/2026-07-21-blade-env-mapping-design.md` | **本项目 dev/test ↔ blade dev/test 环境一一对应（URL 直连，不注册 Nacos）** |
