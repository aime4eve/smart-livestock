...
- **采样率 ≈ 14.5 Hz**（间隔 69ms，毫秒级时间戳）
- 加速度为原始计数（±20000 量级，量纲/量程需对照 MDPI 论文确认，换算前先标定）
- **标签分布：反刍 49.4%（345.9 万行）/ 进食 38.4%（268.5 万行）/ 其他 12.2%（85.3 万行）**——行为覆盖充分，校准统计功效足够
- 校准用途映射：加速度幅值/方差特征 vs 行为标签 → 校准系统 `activityStatus` 阈值（>80/>40/>20 分档）与反刍行为识别；行为节律（反刍 6–8h/天）可交叉验证文献参数
