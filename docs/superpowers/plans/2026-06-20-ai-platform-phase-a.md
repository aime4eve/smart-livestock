...
> **目录与 docker context 说明（评审 #2）**：ai-platform 作为一等微服务（FastAPI 对外端点，非纯 worker），放在 `smart-livestock-server/ai-platform/` 顶层并独立 `build.context: ./ai-platform`，刻意偏离 tile-worker 的 `infrastructure/` + `context: .` 惯例。理由：① ai-platform 体量大（三层架构 + ML 依赖），独立目录边界清晰；② 独立 build context 只打包 `ai-platform/`，不把整个 `smart-livestock-server/`（含 src/build/libs）当上下文，构建更快更干净。
...
> 注：`app` 依赖 ai-platform 的 `depends_on` 移至 **Plan 2**（Java 端联调时才需要，评审 #10）；Plan 1 仅 ai-platform 自身 `depends_on: postgres`，不拖慢 app 启动。
...
Expected: 全部 passed（约 54 个测试，覆盖 schemas/features/detectors/fusion/router/health_l1/engine/db/main/e2e；按单文件累加口径，评审 #12）
...
- iForest 档路由返回但 Phase A 未实现，predict_series 自动降级 mahalanobis（design §4.3 Phase B 激活）；Mahalanobis 档实心。
- CUSUM `history_max` 用历史段滚动最大值（评审 #7 已修正，不再用"当前值×2"退化估计）；真实数据积累后仍需标定归一化尺度（design §4.4）。
