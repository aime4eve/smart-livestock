...
- **GPS 前置依赖**：✅ 已满足。`docs/superpowers/specs/2026-06-26-gps-simulator-consolidation-design.md` 已实施（GpsSimulator 删除、随机游走落地、LivestockInfo 扩展 lastLat/Lng、编译通过）
...
>
> ✅ GPS 收敛已实施（2026-06-26）。SynthesisState 直接搬运 SimulationState 的 `currentLat`/`currentLng` 随机游走逻辑 + `create()` 接收 LivestockInfo 初始化位置。
...
> ✅ 已确认（2026-06-26）：`gps.simulator` 配置段已不存在，GpsSimulator.java 已删除，`rg "GpsSimulator"` 零结果。
