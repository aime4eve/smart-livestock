...
> Spec：`docs/superpowers/specs/2026-09-23-health-integration-final-spec.md`（**v1.1**，已落实评审 F1-F21）
> 原型（视觉蓝本）：`docs/prototypes/health-scenario-alert-integration-prototype.html`
...
- [x] **前置二（F9/F20）**：原型补第七屏（S5 排行页=屏 6）与 S1 展开态画面（屏 1b）——2026-09-23 已补，用户确认通过
- [ ] 全量测试基线记录（19 个既有失败清单）
...
- [ ] 概览页签：告警四卡下加一行四格场景入口（含活跃单角标）+ AI 单行卡（NIX-243 未就绪走空态）+ 对账行（牛级折叠口径，F5）
- [ ] 面板展开态：五格统计迁入（原孪生顶栏数字；**展开态画面已由前置二补齐**）
...
- [ ] build_web + deploy dev + 浏览器走查（对账行数字与告警中心一致）
