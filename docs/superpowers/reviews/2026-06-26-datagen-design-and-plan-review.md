...
### ~~P0 — GPS 三写冲突~~（已撤销 2026-06-26）

> **撤销说明**：此条为误报。默认配置中 `gps.simulator.enabled=false`，`GpsSimulator` 本就不运行。datagen 替换 `TelemetrySimulator` 后行为完全一致——TRACKER readings 照常含 GPS 坐标，走同一条 `ingest()→extractAndLogGps()` 路径。datagen 没有引入任何新的 GPS 问题。两个模拟器的双写风险是**现有代码就有的**，靠配置默认值避免，与 datagen 迁移无关。
...
datagen 的设计方向正确，ACL 隔离和 GroundTruthLabel 实体化的核心决策站得住。但实施计划需要修复 1 个 P0（Task 重排）和 6 个 P1（架构分层不完整 + 运行时风险）才能开始执行。修复都是局部调整，不改变设计方向。建议修复后重新评审计划，再开始 Task-by-Task 实施。
