...
### ~~P0 — GPS 三写冲突~~（重新定性 2026-06-26）

> **重新定性**：GPS 双写风险是真实存在的现有问题——两个模拟器都能写 gps_logs，靠配置默认值避免，无互斥保护。但不是 datagen 引入的：datagen 替换 TelemetrySimulator 后行为完全一致。
>
> 此问题已由独立设计 [GPS 模拟数据收敛设计](./2026-06-26-gps-simulator-consolidation-design.md) 解决：随机游走替代围栏投点 + 删除 GpsSimulator + 统一数据源。datagen-v1 前置依赖：GPS 收敛必须先实施。
...
| # | 问题 | 维度 | 严重度 | 归属 Task |
|---|------|------|--------|-----------|
| ~~1~~ | ~~GPS 三写冲突~~（真实问题，归属 GPS 收敛设计，非 datagen 职责） | 业务拉通 | P0 | GPS 收敛设计先行 |
...
1. ~~GPS 冲突~~（真实问题，已由 GPS 收敛设计解决，datagen 前置依赖）
...
datagen 的设计方向正确，ACL 隔离和 GroundTruthLabel 实体化的核心决策站得住。但实施计划需要修复 1 个 P0（Task 重排）和 6 个 P1（架构分层不完整 + 运行时风险）+ 1 个前置依赖（GPS 收敛设计）才能开始执行。修复都是局部调整，不改变设计方向。建议修复后重新评审计划，再开始 Task-by-Task 实施。
