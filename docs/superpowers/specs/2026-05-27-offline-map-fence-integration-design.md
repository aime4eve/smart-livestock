...
```
创建牧场（画边界围栏）
  → 服务端：Fence(type=boundary) + 检测 tile_regions 覆盖
     ├─ 已覆盖 → farm_tile_tasks(status=ready)
     └─ 未覆盖 → tile_generation_tasks(status=pending) + farm_tile_tasks(status=pending)
...
客户端（WiFi 自动 / 移动网络提示）
  → GET /farms/{id}/tile-status → 检测到可下载
  → 后台下载 MBTiles → 存本地 → SmartTileProvider 离线源可用
...
围栏可以是任意多边形（含近似圆形），tile_regions 始终是矩形（匹配 MBTiles 瓦片网格）。匹配时取围栏顶点的外接矩形（bbox），检查是否被某个 tile_region 的 bbox 覆盖。
...
| `GET /farms/{id}/tile-source` | GET | JWT | 返回该牧场的瓦片源名 + URL |
...
FarmApplicationService.createFarm(name, boundaryVertices)
  ...
  3. 查询 tile_regions: bbox 是否被某个 region 覆盖?
     ├─ 覆盖 → farm_tile_tasks(status=ready, region_id=匹配的region.id)
     └─ 未覆盖 → tile_generation_tasks(status=pending)
                 + farm_tile_tasks(status=pending)
  4. 返回 Farm + tileStatus
```
...
切换牧场时调用 `GET /farms/{id}/tile-source` 获取瓦片源名和 URL，通过 `SmartTileProvider.updateSource(url)` 无缝切换。无瓦片时降级到高德/OSM。
...
class OfflineTileManager {
  ...
  Future<void> startDownload(int farmId, {onProgress, onComplete, onError});
  void cancelDownload(int farmId);
  Future<void> deleteLocalTiles(int farmId);
...
getApplicationSupportDirectory()/mbtiles/{farmId}.meta.json
```
