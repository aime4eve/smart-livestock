...
狭长斜向围栏的 bbox 可能远大于围栏实际面积，导致下载大量无关瓦片。

**覆盖率计算**：`coverageRatio = fencePolygonArea / bboxArea`

| 覆盖率 | 处理策略 |
|--------|---------|
| ≥ 50% | 正常匹配 tile_regions，直接下载 |
| 30%-50% | 正常匹配，但管理 UI 和客户端显示「覆盖率较低」警告，管理员可选择继续或自定义区域 |
| < 30% | 不自动匹配现有 tile_regions，改为创建 `tile_generation_tasks` 生成**围栏形状裁剪后的自定义区域**瓦片 |

**自定义区域生成**：`generate_mbtiles.py` 已支持 `--bbox` 参数指定任意范围。低覆盖率场景下，按围栏 bbox 的**紧凑包围盒**（而非完整 tile_region）生成较小的 MBTiles，减少瓦片数量。生成的 `tile_region` 记录标记为 `status=custom`，仅关联触发创建的牧场。

`tile_generation_tasks` 表新增字段：
```sql
ALTER TABLE tile_generation_tasks ADD COLUMN coverage_ratio DOUBLE PRECISION;
ALTER TABLE tile_generation_tasks ADD COLUMN is_custom_region BOOLEAN NOT NULL DEFAULT false;
```
...
FarmApplicationService.createFarm(name, boundaryVertices)
  ...
  2. 计算 boundaryVertices 的外接矩形 bbox + 覆盖率 coverageRatio
  3. 根据 coverageRatio 分支：
     ├─ ≥50%: 正常匹配 tile_regions → 每个匹配 region 一条 farm_tile_tasks(status=ready)
     ├─ 30%-50%: 匹配 tile_regions + 显示覆盖率警告
     └─ <30%: 创建 tile_generation_tasks(is_custom_region=true, bbox=紧凑包围盒)
              + farm_tile_tasks(status=pending)
  4. 返回 Farm + tileStatus（含关联区域列表 + 覆盖率）
```
