...
> 状态：v1.1 修订版（2026-09-23，逐条落实 spec 评审 F1-F21；原 v1.0 编号 NIX-245 因工单号冲突改为 NIX-246）
> 原型（视觉蓝本）：`docs/prototypes/health-scenario-alert-integration-prototype.html`（八屏：原六屏 + 屏 1b 概览展开态 + 屏 6 AI 排行，2026-09-23 用户确认）
...
| --reconcile-bg / --reconcile-text | rgba(76,154,95,.08) / #3D7D4E（success 派生） | 对账行 |
| --radius-sm/md/lg | 8/12/16px | 圆角三级 |
| --shadow-card / --shadow-sheet | 同告警 V2 | 卡片/面板阴影 |
| 间距 | --xs..--xxl = 4/8/12/16/24/32px | AppSpacing 对齐 |

无全新色相；「新增/迁入/融合」标注色仅评审用，**不进实现**。

## 4. 组件规格（按屏）

**S1 牧场页·概览**：上拉面板三页签不变（概览/围栏/告警）。概览 = 告警四卡（现有）→ 一行四格紧凑场景入口（高 ~50px/格：图标 20px + 名称 9.5pt + 一行状态数字；活跃单角标右上）→ AI 单行卡（🤖 智能健康观察｜关注 N 头｜整体状态词｜"谁最需要关注 ›"；NIX-243 未就绪时空态）→ 对账行（语义见 §5.4）。面板展开态顶部露出五格统计（牲畜总数/健康率/活跃告警/严重异常/设备在线率）——**展开态画面原型未绘制，进 plan 前补画**（裁决 21）。地图标记：有活跃健康单（origin 任一）的牛加红点（数据源见 §5.1 hasActiveHealthAlert）。
...
## 8. 裁决记录（2026-09-23；1-13 为 v1.0 原裁决，14-22 为 v1.1 评审修订）
