...
- **修订记录**：
  ...
  - **r3（2026-09-16，用户裁决）**：交付节奏改为 **P1+P2 全部编码完成 → 一次部署 dev → 一次真机验收**（走一圈实测 + GPX 导入 + 参数调优一轮做完，验收问题修复后补部署复验）；原"P1 先部署验证再做 P2"作废。r3 计划经用户批准即视为 r2 设计批准 + 编码开工。
...
**r3 交付节奏（用户裁决 2026-09-16）**：P1+P2 **全部编码完成**（分支 `nix/213-fence-track-envelope`）→ 编译/单测/analyze 全绿 → **一次** `build_web.sh` + `deploy.sh dev` → **用户一次性真机验收**（沿边走一圈实测、故意横穿内部/漏走一段、GPX 导入、高德瓦片贴合度、L_max/MAD 参数调优）→ 问题修复轮（如需，改后补一次部署复验）→ PR 合并 → 关 NIX-213。实现进展持续更新 NIX-213。
...
3. ~~设计整体批准~~ **已批准**（r3 计划批准时一并生效，2026-09-16）。
4. ~~后端两处防御性校验~~ **做**（按推荐，随本次编码落地）。
5. ~~入口形态~~ **类型下拉加「轨迹采集」项**（按推荐）。
6. ~~P3 提前~~ **不提前**，按兵不动。
