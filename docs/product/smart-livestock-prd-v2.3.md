...
| 版本 | 日期 | 变更 |
|------|------|------|
| v2.3.1 | 2026-09-15 | 设备能力口径更正：智能耳标（ET-100）不支持 GPS 定位，其能力为身份标识 + 体表测温（0.1℃ 精度）+ 步数统计；定位能力仅属 GPS 追踪器（项圈）。同步更正术语表，并同步上市材料与架构图 |
...
- **灵活定价**：设备一次性购买（$65/台，客户所有）+ 订阅按头/月计费（BASIC 免费 ≤50 头，首年免费试点）
...
| 收入线 | 计费模式 | 面向客户 | 计费基础 |
|--------|---------|---------|---------|
| **A. SaaS 订阅** | `direct`（直订） | 牧场主（中大型牧场） | USD 按头/月 × 存栏规模分档（NIX-245） |
| **B. Open API** | `api_usage`（API 用量） | 集成商/智慧农业平台/政府监管 | API 调用次数 × 数据范围 |
| **C. 硬件** | 一次性销售（客户自购、设备归客户） | 所有客户 | **$65/台**（项圈/耳标/胶囊统一）+ 网关按牧场另报 |
...
│      集团管理 | 私有部署 | 定制开发 | 专属支持            │
├─────────────────────────────────────────────────────────┤
│            PREMIUM（专业版）$1.75–3.20/头/月             │
│    发情检测 | 繁殖管理 | 疫病预警 | AI 异常检测          │
├─────────────────────────────────────────────────────────┤
│           STANDARD（标准版）$1.40–2.60/头/月             │
│      定位追踪 | 轨迹回放 | 体温监测 | 健康预警           │
├─────────────────────────────────────────────────────────┤
│                BASIC（免费版）$0                          │
│        ≤50 头 | 定位追踪 | 7 天数据保留                  │
...
> 与后端 `SubscriptionTier` 枚举完全对齐：BASIC、STANDARD、PREMIUM、ENTERPRISE。新用户注册即获 14 天 PREMIUM 全功能试用；正式客户另享 365 天全功能试点（首年免费）。单价随存栏规模分档递减，档位区间见 §3.2。

### 3.2 套餐详细设计

> 定价基准（NIX-245，2026-09-23 批准）：USD 全球统一报价、不含 VAT；硬件 $65/台客户一次性自购（设备归客户）；软件订阅按头/月计费、按存栏规模分档。
...
```
月费($) = 存栏头数 × 所在规模档的单价（$/头/月）
```

| Tier | ＜100 头 | 100–499 头 | ≥500 头 | 示例：260 头牧场 |
|------|---------|-----------|---------|----------------|
| BASIC | $0（≤50 头内） | — | — | — |
| STANDARD | $2.60/头 | $2.15/头 | $1.40/头 | 260×$2.15 = **$559/月** |
| PREMIUM | $3.20/头 | $2.65/头 | $1.75/头 | 260×$2.65 = **$689/月** |
| ENTERPRISE | 定制 | 定制 | 定制 | — |

> 按头计费天然随规模扩展，付费档不再设头数硬上限（原 200/1000 头升档强制已随 NIX-245 移除）；规模分档让大牧场单头成本递减，等价于旧模型的"升档激励"。
...
正式订阅以 5 年期商务合同为主（对齐欧洲市场惯例），续费、折扣均按合同人工确认，系统不再内置固定折扣表（原年付 85 折/两年付 75 折已随 NIX-245 移除）。`billingCycle` 字段保留（monthly/yearly）仅作合同登记。首年免费由 365 天试点授权承担（见 §3.4）。
...
> 按 STANDARD 100–499 档单价 $2.15/头/月计算，毛利率 >90%；新定价体系下单位经济显著优于旧模型。
...
> NIX-245 定价裁决：物联网设备按 **$65/台** 统一报价，客户一次性购买、设备归客户所有（对竞品"硬件摊入订阅的租赁制"的差异化卖点）。LoRaWAN 网关按牧场规模一次性另报（260 头牧场摊薄后 ≈$0.04/头/月，量小但报价单必列）。

| 设备类型 | 售价 | 说明 |
|----------|------|------|
| **动物追踪器（项圈）** | $65/台 | 客户自购，设备归客户 |
| **智能耳标** | $65/台 | 同上（统一价；分型定价留待后续裁决） |
| **瘤胃胶囊** | $65/台 | 同上 |
| **LoRaWAN 网关** | 按牧场另报 | 1 台覆盖 200–500 头 |
| **巡检无人机（外采）** | 项目另报 | 含 SDK 授权 |

> ⚠️ 定价成立前提：设备在栏寿命 ≥5 年（或电池可低成本更换）。若实际寿命 3 年（5 年需两台 $130/头），为维持 5 年总拥有成本 ≈ 竞品 85% 的定位，订阅单价须整体下调（PREMIUM 100–499 档需降至 ≈$2.03/头/月）。

### 3.11 设备 + 订阅组合方案

| 方案 | 设备费用 | 订阅费用 | 适用客户 |
|------|----------|----------|----------|
| **买断模式（现行唯一模式）** | 客户一次性购买 $65/台，设备归客户 | 按头/月订阅 + 首年免费 | 所有客户 |

> 原租赁/混合模式已随 NIX-245 移除：欧洲市场对标（SenseHub GO 硬件全包租赁制）下，"设备买断归客户 + 订阅按头"是我们主动选择的差异化定位，且首年免费期内客户仅承担硬件投入，进入门槛可控。

### 3.12 增值服务包

> 原增值服务包设计（$0.80/$1.40/$2.80 每头每月）已随 NIX-245 并入套餐定价（健康管理/繁殖管理能力包含在 PREMIUM 档），不再单列售卖；未来如需拆分包售卖另行立项。
...
| 场景 | 月收入 | 年收入 |
|------|--------|--------|
| 小型牧场（100 头，STANDARD） | 100×$2.15 = $215 | $2,580 |
| 丹麦典型牧场（260 头，PREMIUM） | 260×$2.65 = $689 | $8,268 |
| 中型牧场（500 头，PREMIUM） | 500×$1.75 = $875 | $10,500 |
| 大型牧场（2,000 头，PREMIUM） | 2,000×$1.75 = $3,500 | $42,000 |

#### 规模化预测

| 客户规模 | 牧场数 | 牲畜总数 | 年订阅收入（$） |
|----------|--------|----------|-----------------|
| 100 头 × 10 牧场（STANDARD） | 10 | 1,000 | 25,800 |
| 260 头 × 20 牧场（PREMIUM） | 20 | 5,200 | 165,360 |
| 2,000 头 × 5 牧场（PREMIUM） | 5 | 10,000 | 210,000 |
| **合计** | **35** | **16,200** | **~401,160** |

> 另有硬件一次性收入：按 1:1 配备，16,200 头 × $65 ≈ **$1,053,000**（一次性）。以上为 SaaS 订阅 + 硬件收入估算，不含 Open API 收入、B2B 分润收入和 Licensed 部署收入；首年免费期（365 天试点）意味着订阅收入从第 13 个月起计入。实际收入随合同折扣策略变化。
...
| 耳标 | 低功耗牲畜身份标识设备，支持体表测温（0.1℃ 精度）与步数统计，不支持 GPS 定位；蓝牙关联防盗 |
