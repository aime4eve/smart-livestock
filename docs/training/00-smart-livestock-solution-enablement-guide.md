...
| 订阅与配额 | basic/standard/premium/enterprise、Feature Gate、用量与升级 | 按阶段采购，避免一次性过度投入 | 定价统一 USD 按头/月 × 规模分档、不含 VAT（NIX-245），无含头数上限 |
...
|---|---|---|---|
| SaaS 直订 `direct` | USD 按头/月 × 存栏规模分档（standard：$2.60/$2.15/$1.40，premium：$3.20/$2.65/$1.75，按 <100/100-499/≥500 头三档；月费 = 存栏头数 × 所在档单价；basic 免费 ≤50 头） | 订阅层级、试用、升级、状态机、配额、Feature Gate | 币种已统一 USD、不含 VAT（NIX-245）；支付、发票和区域商务条款需商务确认；当前 checkout 属于产品流程，不等同于完整海外收款系统 |
...
| 硬件销售 | 一次性客户自购 $65/台（项圈/耳标/胶囊统一价，设备归客户所有）；LoRaWAN 网关按牧场另报 | 设备、设备 License、安装关系、配额和状态管理 | 硬件订单、物流、保修、残值和付款由商务/供应链流程处理，平台不等于订单系统 |
...
| 按头计费 | 月费按存栏头数 × 所在规模档单价计算 | `月费 = 头数 × unitPriceUsdCents`；付费档无头数上限，仅 basic 限 50 头 |
...
> 向客户解释价格前，售前必须先确认付款周期、设备采购、实施、支持服务、SLA 和折扣审批。定价统一 USD 报价、不含 VAT（NIX-245），报价单需注明 VAT 另计；固定折扣表已删除，折扣按商务合同执行。

### 设备与订阅组合

| 组合 | 收入来源 | 适合客户 | 注意事项 |
|---|---|---|---|
| 设备自购 + 平台订阅 | 硬件 $65/台一次性客户自购（项圈/耳标/胶囊统一价，设备归客户所有；LoRaWAN 网关按牧场另报）+ SaaS 按头/月订阅 | 有设备采购预算、希望长期使用平台的牧场 | 明确保修、丢失、维修和续费边界 |
| 客户已有设备 + 平台服务 | SaaS 或集成服务收入 | 已有 GPS/传感器资产的客户 | 先评估设备兼容性和数据获取方式 |
| Enterprise 打包 | 订阅、设备、实施、集成、支持打包定制（买断 ≈36 个月订阅价 + 20%/年维保） | 集团或多牧场客户 | 需技术评估后分项报价 |
| API 数据服务 | API 调用、scope、SLA 或包量收入 | 集成商、平台客户、监管或产业方 | API 商业化口径待确认 |

增值服务包（如健康管理、繁殖管理、全方位服务）的口径已调整：相关能力已并入套餐档位（basic/standard/premium/enterprise），不再作为独立可售 SKU 报价（NIX-245）。
...
海外报价统一 USD、不含 VAT（NIX-245），付款周期、设备采购、实施和支持范围仍需商务确认。历史人民币示例价口径已于 2026-09-23 NIX-245 废止，不得再对外引用。
